...
d3.linegraph = function(noTicks, noDots, parties, partyColors, partyNames, dataMax, dataMin, additionalMonths) {
    ...
    if (!partyColors) {
        partyColors = {'chp': '#E3000F', 'TIP': '#8B0000', 'CGP': '#DCCA4A', 'z': '#000', 'DP': '#750a6f', 'MSP': '#054521', 'MHP': '#954B00', 'other': '#a0a0a0'};
    }
    if (!partyNames) {
        partyNames = {'chp': 'CHP', 'TIP': 'TIP', 'CGP': 'CGP', 'z': 'Z + BVP', 'DP': 'DP', 'MSP': 'MSP', 'MHP': 'MHP', 'other': 'Others'};
    }
    if (!additionalMonths) {
        additionalMonths = 10;
    }
    ...
    function linegraph(dataset) {
     dataset.each(function (data) {
      ...
      if (!dataMax) {
          ...
          const maxMHP = d3.max(data, d => d.MHP);
          dataMax = maxCHP >= maxMHP ? maxCHP + 10 : maxMHP + 10;
          dataMin = 0;
      }
